...
(function (global) {
  ...
  const DEEP = {
    ...
    "states": {
      ...
      "play": {
        "label": "regulated mobilization",
        "body": [
          "You're reporting more regulated mobilization, energy with a little safety mixed in. Expect motivation, focus, easier boundaries, and the kind of drive that doesn't cost you later. Work can count as play here.",
      ...
      "stillness": {
        "label": "regulated immobilization",
        "body": [
          "You're reporting more regulated immobilization, calm and still with a little safety mixed in. Expect a pull toward rest, reflection, and easy closeness. This is a good state for quieter inner work.",
      ...
      "fightflight": {
        "label": "dysregulated mobilization",
        "body": [
          "You're reporting more dysregulated mobilization, a lot of energy without much safety mixed in. Expect frustration, irritability, anxiousness, maybe snapping at people you care about. This is your body trying to handle something, not a character flaw.",
      ...
      "freeze": {
        ...
        "body": [
          ...
          "Freeze showing up in your check-ins, the held-breath, trapped feeling. It's one of the hardest places to be, and it's easy to get overwhelmed here. Small wins count for a lot.",
  ...
  const BLOG_STAGE = {
    orientation: [
      "You're just getting started. Here's what you came in with. → [start here](#2)",
      "First check-in down. Here's where things stand right now. → [start here](#2)"
    ],
    fork_intro: [
      "Let's keep an eye on how your system shapes over time. Here's where a state like this tends to go.",
      "Worth watching how your system takes shape over time. Here's the direction a state like this tends to move."
    ],
    where_bullet: {
      early: ["So far, mostly {STATE}. → [where you've been](#1)", "Your first few check-ins point to {STATE}. → [where you've been](#1)"],
      building: ["Lately, mostly {STATE}. → [where you've been](#1)", "{SHARE} of your check-ins lately were {STATE}. → [where you've been](#1)"]
    },
    section1_where: {
      early: [
        "So far, your check-ins lean toward {STATE}. That's an early look, not the whole picture, you've only checked in a few times. It's the state to start getting to know.",
        "Across your first few check-ins, {STATE} shows up most. It's early yet, but this is the place to start paying attention.",
        "Your first check-ins point toward {STATE}. A few more days will make this clearer. For now, that's the state worth getting to know."
      ],
      building: [
        "These past few days, your check-ins have leaned mostly into {STATE}. Not every moment, but the state your system keeps coming back to lately.",
        "Lately, most of your check-ins land in {STATE}, {SHARE} of them. It's becoming the state to watch.",
        "The past few days have landed mostly in {STATE}. Still early in the picture, but that's where you've been spending your time."
      ]
    },
    movement_building: {
      direction: {
        rising: ["Your safety sits a bit higher lately than when you started checking in. Early, but a good direction. Worth leaning into where you can."],
        falling: ["Your safety sits a little lower lately. Safety comes and goes, and a dip is the 'goes' part. Be a little gentler with yourself over the next few days."],
        steady: ["Your safety has held pretty level the past few days. A steady stretch is solid ground, especially this early."]
      },
      variance: {
        shifts: ["Your check-ins have moved around these past few days rather than sitting still. A system that moves is doing what it's supposed to."],
        consistent: ["Your check-ins have stayed in a similar place the past few days. If that place feels okay, that's steady ground; if it feels hard, it's worth gently working your way out."]
      }
    },
    section6_note: {
      start: [
        "That's one check-in. Not much to go on yet, but no worries, because this page grows and adapts based on your check-ins. Come back after the next one and check for yourself!"
      ],
      early: [
        "A few check-ins in, and you're already starting to watch your own system. You're not grading yourself, you're getting to know yourself. Keep showing up.",
        "You've checked in a handful of times now. That's how the picture builds, one honest check-in at a time. Each one makes the next clearer."
      ],
      building: [
        "A few days in, and a shape is starting to show. You're not grading yourself, you're getting to know yourself. Keep going the way you have been.",
        "You've been checking in for a few days now. The picture's still filling in, and it's already yours. Keep showing up the way you have been."
      ],
      returning: [
        "Good to see you back. It's been a few days, so this leans on your last check-in or two. Keep going and the fuller picture comes back quickly.",
        "Welcome back. After a break, this is lighter for now, working from your most recent check-ins. A few more days and the shape fills back in."
      ],
      milestone: [
        "A full week in. This is your first stretch with a real week behind it, more signal, less guesswork. Nice work showing up for it.",
        "You've checked in across a whole week now. That's the point where this starts telling you something real. Worth noticing how far you've come to get here."
      ],
      established: [
        "You've been at this a while now, and it shows, you notice your own states more easily than when you started. Keep going.",
        "Weeks of check-ins behind you now. The picture's yours, and it keeps getting clearer the longer you stay with it. Keep showing up the way you have been."
      ]
    }
  };
  ...
  function blog(ctx0){
    ...
    if(stage==='start'){
      ...
      s0.push({ id:'blog-2', heading:"what that state is", paras:[ cycle('s2what:'+dom, r0.what_this_is), r0.how_it_shows_up ]});
      const fk0=BLOG.section4_fork[dom];
      if(fk0) s0.push({ id:'blog-4', heading:"the fork ahead", paras:[ cycle('forkintro:start', BLOG_STAGE.fork_intro), fk0.toward_regulated, fk0.if_reps_drop, fk0.landing ].filter(Boolean) });
    ...
    if(!thin && ctx.dir && BLOG.tldr_bullets.direction[ctx.dir]) bullets.push(_bullet(P('dirb', BLOG.tldr_bullets.direction[ctx.dir]), ctx));
    if(!thin && ctx.variance && BLOG.tldr_bullets.variance[ctx.variance]) bullets.push(_bullet(P('varb', BLOG.tldr_bullets.variance[ctx.variance]), ctx));
    if(!thin) bullets.push(_bullet(P('forkb', BLOG.tldr_bullets.fork), ctx));
